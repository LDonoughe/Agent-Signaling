				...
				int * itno)
{
	...
	while(c != (char)EOF)
	{
		...
		if(c == '>')
		{
			...
			if(strcmp(buffer, "/xagent") == 0)
			{	
				if(strcmp(agentname, "buyer") == 0)
				{
					if(current_buyer_agent == NULL) { printf("Memory error reading buyer agent\n"); exit(0); }
					...
					if( flag == 0 )
					{
						...
						add_buyer_agent_internal(current_buyer_agent, buyer_start_state);
			...
			if(strcmp(buffer, "my_id") == 0) { in_my_id = 1; }
			if(strcmp(buffer, "/my_id") == 0) { in_my_id = 0; }
			if(strcmp(buffer, "firm_id") == 0) { in_firm_id = 1; }
			if(strcmp(buffer, "/firm_id") == 0) { in_firm_id = 0; }
			if(strcmp(buffer, "my_qual") == 0) { in_my_qual = 1; }
			if(strcmp(buffer, "/my_qual") == 0) { in_my_qual = 0; }
			if(strcmp(buffer, "buyer_ids") == 0) { in_buyer_ids = 1; }
			if(strcmp(buffer, "/buyer_ids") == 0) { in_buyer_ids = 0; }
		...
		else if(c == '<')
		{
			...
			if(FLAME_in_xagent == 1)
			{
				if(FLAME_in_name == 1)
				{
					...
					if(strcmp(agentname, "buyer") == 0)
					{
						current_buyer_agent = init_buyer_agent();
						in_buyer_agent = 1;
					}
					else if(strcmp(agentname, "firm") == 0)
					{
				...
				else if(in_buyer_agent == 1)
				{
					if(in_my_id) { current_buyer_agent->my_id = atoi(buffer); }
					if(in_firm_id) { current_buyer_agent->firm_id = atoi(buffer); }
					if(in_my_qual) { current_buyer_agent->my_qual = atoi(buffer); }
				 }else if(in_firm_agent == 1)
				{
					if(in_buyer_ids) { j = 0;
						rc = read_int_static_array(buffer, index, &j, current_firm_agent->buyer_ids, 100);
						if(rc != 0) { printf("Error: reading 'firm' agent variable 'buyer_ids' of type 'int'\n"); exit(0); } }
...
void write_buyer_agent(FILE *file, xmachine_memory_buyer * current)
{
	char data[1000];
	fputs("<xagent>\n" , file);
	fputs("<name>buyer</name>\n", file);
		fputs("<my_id>", file);
	sprintf(data, "%i", current->my_id);
	fputs(data, file);
	fputs("</my_id>\n", file);
		fputs("<firm_id>", file);
	sprintf(data, "%i", current->firm_id);
	fputs(data, file);
	fputs("</firm_id>\n", file);
		fputs("<my_qual>", file);
	sprintf(data, "%i", current->my_qual);
	fputs(data, file);
	fputs("</my_qual>\n", file);
...
void write_firm_agent(FILE *file, xmachine_memory_firm * current)
{
	...
	fputs("<name>firm</name>\n", file);
		fputs("<buyer_ids>", file);
	write_int_static_array(file, current->buyer_ids, 100);
	fputs("</buyer_ids>\n", file);
		fputs("<my_id>", file);
	sprintf(data, "%i", current->my_id);
	fputs(data, file);
	...
	fputs(data, file);
	fputs("</quality>\n", file);
		fputs("<stored_id>", file);
	sprintf(data, "%i", current->stored_id);
	fputs(data, file);
	fputs("</stored_id>\n", file);
...
void FLAME_write_xml(char * location, int iteration_number, int * output_types, int output_type_size)
{
	...
	if(FLAME_integer_in_array(0, output_types, output_type_size) || FLAME_integer_in_array(1, output_types, output_type_size))
	{
		current_xmachine_buyer_holder = buyer_start_state->agents;
			...
			{
				write_buyer_agent(file, current_xmachine_buyer_holder->agent);

				current_xmachine_buyer_holder = current_xmachine_buyer_holder->next;
			}
	}
